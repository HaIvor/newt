...
def calculateHessianAndGradient(xi):
    ...
    constraint6 = sym.log(-(sym.log(x3)+sym.log(R_n) + sym.log(x1) + sym.log(sym.log(x2,2)) + (1/R_c)*(sym.log(0.2)-(3*gamma)/(2*(x2-1))) - sym.log(p_lt) )  )
    ...
    function = bb*( function_without_constraint ) - (constraint1+constraint2+constraint3 + constraint6) 
...
# gradient, zi = calculateHessianAndGradient(xi)
# yi = zi@xi-gradient
...
# gradient, zj = calculateHessianAndGradient(xj)
# yj = zj@xj-gradient
...
i = 1
epsilon = 0.00001 # just to give value temporary
bb=1
while i < 1000: 
...
# ------Data transmission-------
    ...
    if flag_transmission == 1:
        ...
        sigmai_y_sendt = np.float16(sigmai_y)
        sigmai_z_sendt = np.float16(sigmai_z)
...
# ------Data Reception------
    ...
    if flag_reception == 1:
        ...
        yi = yi + sigmaj_y_sendt - rhoi_y
        zi = zi + sigmaj_z_sendt - rhoi_z
        rhoi_y = sigmaj_y_sendt
        rhoi_z = sigmaj_z_sendt
        flag_reception, flag_update, flag_transmission, flag_update2, flag_reception2, flag_transmission2 = 0, 0, 0, 0, 0, 0
        flag_update = 1
...
#-----------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    if flag_transmission2 == 1:
        ...
        sigmaj_y_sendt = np.float16(sigmaj_y)
        sigmaj_z_sendt = np.float16(sigmaj_z)
        #pseudo code: Broadcast: transmitter_node_ID, sigma_y[i], sigma_z[i].
        # Have to set up with modem. Subnero stuff...
        flag_reception, flag_update, flag_transmission, flag_update2, flag_reception2, flag_transmission2 = 0, 0, 0, 0, 0, 0
        flag_reception = 1
    ...
    if flag_reception2 == 1:
       ...
       # Will come from neighbor node (node j)
        ...
        yj = yj + sigmai_y_sendt - rhoj_y #andre noden er nr "1"
        zj = zj + sigmai_z_sendt - rhoj_z
        rhoj_y = sigmai_y_sendt
        rhoj_z = sigmai_z_sendt
        flag_reception, flag_update, flag_transmission, flag_update2, flag_reception2, flag_transmission2 = 0, 0, 0, 0, 0, 0
        flag_update2 = 1
