...
def cost_function(x):
    ...
    return -(log(x[2])+log(R_c)+log(B)+log(R_n)+log(x[0])+log(log2(x[1]))-log(x[2]*(1+p_c)*x[0]+B*(t_oh+t_d))) - (  log(-(x[0]-const_N)) + log(-(x[2]-const_m) ) + log(-(x[1]-const_M)) + log(-(16-x[0]))+ log(-(2-x[1]))+ log(-(   log(x[1])+log(x[0])+log(R_n)+log(log2(x[1])) + (1/R_c)*(log(0.2) - (3*gamma)/(2*(x[1]-1))) - log(p_lt)    ))) 
