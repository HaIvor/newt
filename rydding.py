...
gradient, hi = np.zeros((n, 1)), np.identity(n)
k=1
...
while i < iterations:
    ...
    while k:
        hi = nd.Hessian(cost_function)(xi.reshape((2,))) 
        gradient = nd.Gradient(cost_function)(xi).reshape((2,1)) 
        deltax = -1.0*(np.linalg.inv(hi)@gradient)
        lambda_2  = np.transpose(gradient)@np.linalg.inv(hi)@gradient

        if (lambda_2[0][0]/2) < 1e-5:
             print("break")
             print("lambda/2 was :", lambda_2[0][0]/2)
             break
        
        p = 1
        while cost_function(xi+p*deltax) > (cost_function(xi)+alfa*p*np.transpose(gradient)@deltax):
            p = p*beta
            print("p blitt byttet til: ", p)

        xi = xi + p*deltax
          
        #print(cost_function(xi+p*deltax))
        #print((cost_function(xi)+alfa*np.transpose(gradient)@deltax))   
