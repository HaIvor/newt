...
def calculateHessianAndGradient(xi):
    ...
    constraint1 = sym.log(-(x3-(4*B)/((1+p_c)*x1))) # m < 4B/(1+pc)*N
    constraint2 = sym.log(-(1-x3)) # m > 1.1
    constraint3 = sym.log(-(x1-600)) # N < 600
    constraint4 = sym.log(-(10-x1)) # N > 10
    constraint5 = sym.log(-(2-x2)) # M > 2.1
    constraint6 = sym.log(  -(sym.log(x2)+sym.log(R_n) + sym.log(x1) + sym.log(sym.log(x2,2)) + (1/R_c)*(sym.log(0.2)-(3*gamma)/(2*(x2-1))) - sym.log(p_lt) )  )
    constraint_tull = sym.log(-(x3-70)) # m < 70
    constraint_tull2 = sym.log(-(x2-64))# M < 64
    constraint_tull3 = sym.log(-(x1-700)) # N < 700
    function = t*( -(sym.log(x3)+ sym.log(R_c) + sym.log(B) + sym.log(R_n) + sym.log(x1) + sym.log(sym.log(x2, 2)) - sym.log(x3*(1+p_c)*x1 + B*(t_oh + t_d)))) - (constraint5+constraint2+constraint4+constraint3+constraint6+constraint1) 
